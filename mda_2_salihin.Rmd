...
2. Regime type -- democracy, non-democracy (e_boix_regime)
3. Democratic transition -- democratic breakdown, no change, democratic transition (e_democracy_trans)
4. Status -- free, partly free, or not free (e_fh_status)
5. GDP per capita -- (e_migdppc)
6. Average years of education -- (e_peaveduc)
7. Country land area -- (e_area)
...
vdem_subset <- vdem %>%
  filter(year == 2015) %>%
  filter(!is.na(e_boix_regime)) %>%
  select(country_name, e_boix_regime, e_democracy_trans, e_fh_status, e_migdppc, e_peaveduc, e_area, v2x_polyarchy)
...
vdem_subset_untidy <- vdem_subset_tidy %>%
  pivot_wider(id_cols = country_name,
              ...
              values_from = c(e_democracy_trans, e_fh_status, e_migdppc, e_peaveduc, e_area, v2x_polyarchy),
              names_sep = "^"
              )
...
vdem_subset_tidy_again <- vdem_subset_untidy %>% 
  pivot_longer(cols = -country_name, 
               names_to  = c(".value", "democracy"),
               names_sep = "^")

# c(".value", "e_boix_regime", "e_democracy_trans", "e_fh_status", "e_migdppc", "e_peaveduc", "e_area", "v2x_polyarchy")
